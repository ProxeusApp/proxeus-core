!(function (a, b) {
  ...
  'use strict'
  var b = function (c, d) {
    this.$element = a(c), this.options = a.extend({}, b.defaults, d), this.matcher = this.options.matcher || this.matcher, this.sorter = this.options.sorter || this.sorter, this.select = this.options.select || this.select, this.autoSelect = typeof this.options.autoSelect !== 'boolean' || this.options.autoSelect, this.highlighter = this.options.highlighter || this.highlighter, this.render = this.options.render || this.render, this.updater = this.options.updater || this.updater, this.displayText = this.options.displayText || this.displayText, this.source = this.options.source, this.delay = this.options.delay, this.$menu = a(this.options.menu), this.$appendTo = this.options.appendTo ? a(this.options.appendTo) : null, this.fitToElement = typeof this.options.fitToElement === 'boolean' && this.options.fitToElement, this.shown = !1, this.listen(), this.showHintOnFocus = (typeof this.options.showHintOnFocus === 'boolean' || this.options.showHintOnFocus === 'all') && this.options.showHintOnFocus, this.afterSelect = this.options.afterSelect, this.addItem = !1, this.value = this.$element.val() || this.$element.text()
  }
  b.prototype = {
    constructor: b,
    select: function () {
    ...
    show: function () {
      var d; var b = a.extend({}, this.$element.position(), { height: this.$element[0].offsetHeight })
      var c = typeof this.options.scrollHeight === 'function' ? this.options.scrollHeight.call() : this.options.scrollHeight
      ...
      var f = a(d).parent().hasClass('dropup'); var g = f ? 'auto' : b.top + b.height + c
      var h = a(d).hasClass('dropdown-menu-right'); var i = h ? 'auto' : b.left
      return d.css({
        top: g,
        left: i
    ...
    process: function (b) {
      ...
      return b = a.grep(b, function (a) {
        return c.matcher(a)
      }), b = this.sorter(b), b.length || this.options.addItem ? (b.length > 0 ? this.$element.data('active', b[0]) : this.$element.data('active', null), this.options.addItem && b.push(this.options.addItem), this.options.items == 'all' ? this.render(b).show() : this.render(b.slice(0, this.options.items)).show()) : this.shown ? this.hide() : this
    },
    matcher: function (a) {
    ...
    render: function (b) {
      var c = this; var d = this; var e = !1; var f = []; var g = c.options.separator
    ...
    next: function (b) {
      var c = this.$menu.find('.active').removeClass('active'); var d = c.next()
      d.length || (d = a(this.$menu.find('li')[0])), d.addClass('active')
    },
    prev: function (a) {
      var b = this.$menu.find('.active').removeClass('active'); var c = b.prev()
  ...
  a.fn.typeahead = function (c) {
    ...
    return typeof c === 'string' && c == 'getActive' ? this.data('active') : this.each(function () {
      var e = a(this); var f = e.data('typeahead'); var g = typeof c === 'object' && c
