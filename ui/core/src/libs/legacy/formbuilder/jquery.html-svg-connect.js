  ...
  $.extend(Plugin.prototype, {
    ...
    connectSetup: function (pathConfig) {
      if (pathConfig.hasOwnProperty('start') && pathConfig.hasOwnProperty('end')) {
        var $start = $(pathConfig.start); var $end = $(pathConfig.end)
        ...
        if ($start.length && $end.length && $start.is(':visible') && $end.is(':visible')) {
          ...
          if (pathConfig.startPosition) {
            pathData.startPosition = pathConfig.startPosition
          }
          if (pathConfig.endPosition) {
            pathData.endPosition = pathConfig.endPosition
          }
    ...
    connectElements: function (pathData) {
      ...
      var $endElem = pathData.end
      var orientation = pathData.orientation
      ...
      var svgTop = this.$parent.offset().top
      var svgLeft = this.$parent.offset().left
      ...
      var centreSX = 0.5; var centreSY = 1
      var centreEX = 0.5; var centreEY = 0
      if (orientation == 'vertical') {
        centreSX = 1
        centreSY = 0.5
        centreEX = 0
      ...
      // // console.log("pathData: "+pathData.startPosition);
      // if(pathData.startPosition){
      //     if(pathData.startPosition=="vertical"){
      //         centreSX = 0.5;centreSY = 1;
      //     }else if(pathData.startPosition=="horizontal"){
      //         centreSX = 1; centreSY = 0.5;
      //     }
      // }
      // // console.log("pathData: "+pathData.endPosition);
      // if(pathData.endPosition){
      //     if(pathData.endPosition=="vertical"){
      //         centreEX = 0.5; centreEY = 0;
      //     }else if(pathData.endPosition=="horizontal"){
      //         centreEX = 0; centreEY = 0.5;
      //     }
      // }
      ...
      this.drawPath(pathData, pathData.path, pathData.offset, orientation, startX, startY, endX, endY)
    },

    drawPath: function (pathData, $path, offset, orientation, startX, startY, endX, endY) {
      ...
      if (orientation == 'vertical') {
        ...
        // if(pathData.startPosition || pathData.endPosition){
        // }else{
        // Draw the pipe-like path
      ...
      if (pathData.animate) {
        this.animate($path[0], pathData.animate)
      }
    },
    ...
    animate: function (p, a) {
      ...
      p.style.strokeDashoffset = length * -1
      p.getBoundingClientRect()
      var speed = typeof a === 'string' ? a : '1.0s'
      p.style.transition = p.style.WebkitTransition = 'stroke-dashoffset ' + speed + ' ease-in-out'
      // Go!
  ...
  $.fn[pluginName] = function (options) {
    ...
    if (options === undefined || typeof options === 'object') {
      ...
      return this.each(function () {
        var t = $.data(this, 'plugin_' + pluginName)
        if (!t) {
          $.data(this, 'plugin_' + pluginName, new Plugin(this, options))
        } else {
          t.setup(this, options)
        }
      })
