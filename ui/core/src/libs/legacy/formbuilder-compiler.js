...
var FT_FormBuilderCompiler = function (options) {
  ...
  this.registerTemplateCompilerFeatures = function () {
    ...
    Handlebars.registerHelper('enumEq', function (a, b, opts) {
      if (_c.isEnum(a)) {
        if (a.all[a.selected] === b) { return opts.fn(this) } else { return opts.inverse(this) }
      }
    })
    ...
    Handlebars.registerHelper('dropHere', function (opts) {
      ...
      try {
        ...
        dropHerePH = '<div class="fb-drop-here" data-index="' + dropHereIndex + '"></div>'
          ...
          if (cp) {
            ...
            if (mp['_compId']) {
              ...
              if (this.c.options.injectFormCompiler) {
                ...
                if (o.i18n && o.i18n.onDisplay && o.i18n.onTranslate && o.i18n.isCovered) {
              ...
              return this.c.compileAndSetIdOnRootElement(mp['_compId'], cp, cmpId)
            }
          }
        }
      }
  ...
  this.cForm = function (data) {
    ...
    var _ = this; var sortedComps = []; var currentComp; var c; var compMainObj; var compActions = {}; var template; var id; var compiledComp; var allCompsCompiled = ''; var validationExt = {}
    ...
    sortedComps.sort(function (a, b) { return _.getCompMainObject(a)['_order'] - _.getCompMainObject(b)['_order'] })
    ...
    for (var i = 0; i < sortedComps.length; i++) {
      ...
      if (compMainObj['_compId']) {
        compiledComp = this.compileAndSetIdOnRootElement(compMainObj['_compId'], currentComp, id)
        if (data.onCompCompiled) {
          compiledComp = data.onCompCompiled(compiledComp, compMainObj, currentComp)
        }
        if (compiledComp) {
    ...
    if (!allCompsCompiled) { return '' }
    htmlStr += allCompsCompiled
    if (!data.componentsOnly) {
      var fmf = this.FORM_CONTEXT_VAR_NAME
      htmlStr +=
                '   <script type="text/javascript">\n' +
                '       if(!window["' + fmf + '"]){window["' + fmf + '"]={}}' +
                '       window["' + fmf + '"]["' + formId + '"]= ' + this.formRuntimeMain.toString() + ';\n' +
                '       window["' + fmf + '"]["' + formId + '"]=window["' + fmf + '"]["' + formId + '"]();\n' +
                '       window["' + fmf + '"]["' + formId + '"].main("' + fmf + '","' + formId + '",' + JSON.stringify(compActions, null, 2) + ',' + JSON.stringify(validationExt, null, 2) + ');\n' +
                '   </script>' +
                '</form>'
    }
  ...
  this.compileAndSetIdOnRootElement = function (compId, compSettings, htmlId) {
    ...
    if (!cachedTmpl) {
      ...
      if (!c || !c['template']) {
        ...
        if (this.options.injectFormCompiler && compId && htmlId) {
          return '<div id="' + htmlId + '" data-dfsid="' + compId + '" class="fb-component"><span style="color:red;padding:20px;display:inline-block;">Error could not render this component! Looks like it does not exist anymore.</span></div>'
        }
        return
      }
      template = c['template']
    ...
    if (compiledComp) {
      ...
      if (this.options.injectFormCompiler) {
        return this.addClassOnRootElement(this.addAttrOnRootElement(compiledComp, 'data-dfsid', compId), cachedTmpl.isGrpParent ? 'fb-component fbc-grp-parent' : 'fb-component')
      } else {
        return compiledComp
      }
    } else {
  ...
  this.cacheTemplate = function (id, tmpl) {
    ...
    return this.preCompiledCache[id] = { tmpl: Handlebars.compile(tmpl), isGrpParent: tmpl.indexOf('{{dropHere}}') !== -1 }
  }
      ...
      if (show) {
        if (!this.is(':visible')) {
          if (init) {
            ...
            $form.trigger('action-animation-started', [{ init: init, show: show, target: this }])
            this.css('opacity', 0)
            this.slideDown(250, 'swing', function () {
              $(this).animate({ opacity: 1 }, 400, function () {
                $(this).css('opacity', 1)
                $form.trigger('action-animation-ended', [{ init: init, show: show, target: this }])
              })
            })
          }
        }
    ...
    $.fn.fm_animate = function ($form, init, show, $caller) {
      ...
      if (show) {
              ...
              for (var i = 0; i < srcs.length; ++i) {
                ...
                if (innersrcs) {
                  for (var ss = 0; ss < innersrcs.length; ++ss) {
                    $form.find("[name='" + srcs[i].name + "']:eq(" + innersrcs[ss]._index + ')').doCompAction(init)
                  }
                }
              }
            }
        ...
        for (var id in fmShow.show) {
          if (fmShow.show.hasOwnProperty(id) && fmShow.show[id]['active'] && !$form.find('#' + fmShow.show[id]['parent']).data('fm_hiding')) {
          ...
          try {
            ...
            if (srcs) {
              ...
              for (var i = 0; i < srcs.length; ++i) {
                ...
                if (innersrcs) {
                  for (var ss = 0; ss < innersrcs.length; ++ss) {
                    $.fn.fm_animate.apply($form.find('#' + innersrcs[ss]._destCompId), [$form, init, show])
                  }
                }
              }
            }
      ...
      $form.trigger('action-change', [{ init: init, show: show, target: this, caller: $caller }])
    ...
    return {
      main: function (formContextVar, formId, actions, validationExt) {
        ...
        $targetForm.parent().bind('DOMNodeRemoved', function (e) {
          try {
            if (e && e.target && window[formContextVar][formId].form === e.target) {
        ...
        var currentComp, currentCompItem, sources, $srcComp, destCompId, $destComp, $fields, $targetField
            ...
            if ($srcComp.length) {
              for (var i = 0; i < currentComp.length; ++i) {
                ...
                if (currentCompItem.action.source) {
                  ...
                  $fields = $targetForm.find("[name='" + currentCompItem.name + "']")
                  if ($fields && $fields.length) {
                          ...
                          if ($destComp.length) {
                            ...
                            if ($targetField && $targetField.length) {
                              if (!$targetField.data('fm_actionData')) {
                                ...
                                fmShow.show[currentCompItem.name]['parent'] = id
                                fmShow.show[currentCompItem.name]['active'] = false
                                $destComp.hide()
                                $targetField.data('fm__index', sources[s]['_index'])
                                $targetField.data('fm_actionData', [sources[s]])
                                $targetField.data('fm_actionDataAll', sources)
                                $targetField.data('fm_action', this.actionEvent)
                              } else {
                                var fmShow = $destComp.data('fm_show')
                                if (!fmShow) {
                                  fmShow = { show: {} }
                                ...
                                fmShow.show[currentCompItem.name]['active'] = false
                                $destComp.hide()
                                $targetField.data('fm_actionData').push(sources[s])
                              }
                            }
                          }
                        }
                    ...
                    if (this.isRadio($($fields[0])) || this.isRadio($($fields[$fields.length - 1]))) {
                      var $cRorC
                      var cRorCIndex = 0
                      for (var rc = 0; rc < $fields.length; ++rc) {
                        $cRorC = $($fields[rc])
                        if (this.isRadio($cRorC)) {
                          if (!$cRorC.data('fm__index') && !$cRorC.data('fm_actionDataAll') && !$cRorC.data('fm_action')) {
                ...
                if (currentCompItem.action.destination) {
                  ...
                  if (d.transition &&
                    ...
                    $srcComp.data('fm_transition', d.transition.all[d.transition.selected])
                  }
                }
              }
            }
      ...
      createRegex: function (r) {
        ...
        var rStart = regexStart.exec(r); var rEnd = regexEnd.exec(r)
  ...
  this.addAttrOnRootElement = function (htmlStr, attrName, attrValue) {
    ...
    if (m !== null && m.length) {
      ...
      rootStartTag = rootStartTag.replace(new RegExp('\ ' + attrName + '(\=(\"|\')(.*?)(\"|\'))?', 'g'), '')
      rootStartTag = rootStartTag.replace(/\s*$/, '') + ' ' + attrName + '="' + attrValue + '"'
  ...
  this.addClassOnRootElement = function (htmlStr, classVal) {
    ...
    if (m !== null && m.length) {
      ...
      if (m !== null && m.length) {
        ...
        rootStartTag = rootStartTag.replace(/(\ class(\=(\"|\')(.*?)(\"|\'))?)/, '')
      }
      rootStartTag = rootStartTag.replace(/\s*$/, '') + ' class="' + classVal + '"'
      htmlStr = htmlStr.replace(rootStartTagReg, rootStartTag)
    }
    return htmlStr
  }
  this.translateSettings = function (json, tmplId, i18nObj) {
    ...
    if (json) {
      ...
      if (keyArray.length > 0) {
        i18nObj.onTranslate(keyArray, function (translatedArray) {
  ...
  this.getFormCompilerForInjection = function () {
    if (!this.formCompilerIdForInjection) {
      if (this.options.formCompiler && this.options.formCompiler.getComponentsHolder) {
        this.formCompilerIdForInjection = this.randomId()
        this.fcc[this.formCompilerIdForInjection] = new this.FormCompilerClass(this, this.options.formCompiler.getComponentsHolder())
      }
    }
  ...
  this.deepCopySettings = function (settingsOfComp, settingsToMergeInto) {
    if (settingsToMergeInto) {
      ...
      } else if (!this.isArray(settingsToMergeInto) && this.isArray(settingsOfComp)) {
        settingsToMergeInto = [settingsToMergeInto]
      }
      return this.extend(true, this.isArray(settingsOfComp) ? [] : {}, settingsOfComp, settingsToMergeInto)
    }
    return this.extend(true, this.isArray(settingsOfComp) ? [] : {}, settingsOfComp)
  }
  try {
  ...
  this.cleanActionSrc = function (comp) {
    if (comp && comp.action && comp.action.source && comp.action.source.length === 0) {
  ...
  this.isEnum = function (o) {
    return o && o.hasOwnProperty('selected') && o.hasOwnProperty('all') && o.all.length > 0
  }
