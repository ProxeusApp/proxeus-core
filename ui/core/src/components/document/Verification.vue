  ...
  <div>
    <div class="verification animscroll mt-2 col-lg-10 col-lg-offset-1">
      <div class="animscroll">
        <spinner
          v-show="!wallet || initialized === false"
          background="transparent"
          color="#eee"
        ></spinner>
        <div
          class="light-text mx-auto mt-5"
          v-show="!wallet || initialized === false"
        >
          {{ $t("Connecting with the blockchain ...") }}
        </div>
        <div
          class="dropbox bg-white pl-0"
          v-if="initialized && !hasPreinitError"
          :class="{ hasDocuments }"
        >
          ...
          <p class="w-100 text-center" v-if="!hasDocuments">
            <span class="text-hint py-2 px-4 d-inline-block">
              {{
                $t(
                  "Verify file here",
                  "You can verify the authenticity of your document here. The hash of your document will be compared to the hashes, which were registered on the blockchain upon document creation."
                )
              }}
            </span>
          </p>
        ...
        <div
          class="stats col-sm-12 p-0"
          v-show="hasDocuments && !hasPreinitError"
        >
