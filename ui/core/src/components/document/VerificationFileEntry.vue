  ...
  <div class="verification-file-entry" v-bind="$attrs">
    <div v-if="singleFile" class="text-center p-3 border border-2 border-light">
      <div class="pb-1">
        ...
        <i
          v-show="valid"
          class="text-success material-icons md-60 mdi mdi-check-circle"
        ></i>
        <i
          v-show="notFound"
          class="text-danger material-icons md-60 mdi mdi-close-circle"
        ></i>
        <i
          v-show="invalid || validationException"
          class="text-danger material-icons md-60 mdi mdi-alert-circle"
        ></i>
      </div>
      <div v-if="!loading">
        <div class="break-word mx-auto mt-2 mb-0">
          <h4 v-show="valid" class="text-success">
            {{
              $t("Verified file is valid", "The file {filename} is valid.", {
                filename: file?.name,
              })
            }}
          </h4>
          <h4 v-show="invalid" class="text-danger">
            {{
              $t(
                "Verified file revoked",
                "The file {filename} has been revoked.",
                { filename: file?.name }
              )
            }}
          </h4>
          <h4 v-show="notFound" class="text-danger">
            {{
              $t(
                "Verified file is invalid",
        ...
        <div class="text-center mt-2">
          <div v-show="valid" class="mb-2">
            ...
            <a
              class="break-word font-weight-bold"
              :title="creator"
              target="_blank"
              :href="'https://' + network + 'etherscan.io/address/' + creator"
              v-if="contract"
            >
          ...
          <div v-if="showDetails === true || !valid">
            ...
            <div class="container text-md-left text-center">
              <div v-show="timestamp" class="row mb-2">
                <div class="offset-md-2 col-md-3 pr-0 text-hint">
                  {{ $t("Verified file timestamp", "Registration date:") }}
                </div>
                <div class="col-md-5">{{ timestamp }}</div>
              </div>
              <div v-show="valid && hash" class="row mb-2">
                <div class="offset-md-2 col-md-3 pr-0 text-hint">
                  {{ $t("Verified file hash", "File Hash:") }}
                </div>
                <div class="col-md-5 break-word">
                  {{ hash | addressOrHash }}
                </div>
              </div>
              <div class="row mb-2" v-show="contract">
                <div class="offset-md-2 col-md-3 pr-0 text-hint">
                  {{ $t("Verified file contract addr", "Contract Address:") }}
                </div>
                <div class="col-md-5">
                  <a
                    class="break-word"
                    ...
                    target="_blank"
                    :href="
                      'https://' + network + 'etherscan.io/address/' + contract
                    "
                    v-if="contract"
                  >
              ...
              <div class="row" v-show="tx">
                <div class="offset-md-2 col-md-3 pr-0 text-hint">
                  {{ $t("Verified file transaction", "Transaction:") }}
                </div>
                <div class="col-md-5">
                  <a
                    class="break-word"
            ...
            <div
              class="text-hint w-100 px-md-5 mt-2"
              :class="{ 'pt-2': valid, 'pt-0': !valid }"
            >
              <p v-show="valid">
                {{
                  $t(
                    "Verified file is valid 2",
                    "This file has been recognized as genuine and valid."
                  )
                }}
              </p>
              <p v-show="notFound">
                {{
                  $t(
                    "File not registered explanation",
              ...
              <p v-show="invalid">
                {{
                  $t(
                    "Verified file invalid explanation",
              ...
              <p v-show="errorValidating">
                {{
                  $t(
                    "Error while verifying file",
                    "File hash could not been verified due to technical problems. Please try again."
                  )
                }}
              </p>
              <p v-show="errorPublicRPC">
                {{
                  $t(
                    "Error while using public RPC",
                    "Please install Metamask extension to verify file."
                  )
                }}
              </p>
            </div>
        ...
        >
          <i
            class="material-icons md-16 mdi"
            :class="{
              'mdi-arrow-down-drop-circle-outline': !showDetails,
              'mdi-arrow-up-drop-circle-outline': showDetails,
            }"
          ></i>
    ...
    <div
      v-else-if="hasPreinitError"
      class="text-center p-3 border border-2 border-light"
    >
      <div class="pb-1">
        ...
        <div class="text-center mt-2">
          <div>
            <div class="text-hint w-100 px-md-5 mt-2 pt-0">
              <p v-show="isIncorrectNetwork">
                {{
                  $t(
                    "Error incorrect network",
                    "Document can not be verified within incorrect network."
                  )
                }}
              </p>
            </div>
      ...
      <div class="row list-item">
        <div class="bg-light col-lg-1 px-1 status-holder text-center">
          <div class="check py-2">
            ...
            <i
              v-show="notFound"
              class="text-danger material-icons md-36 mdi mdi-close-circle"
            ></i>
            <i
              v-show="invalid || validationException || isIncorrectNetwork"
              class="text-danger material-icons md-36 mdi mdi-alert-circle"
            ></i>
            <i
              v-show="loading === false && status"
              class="text-success material-icons md-36 mdi mdi-check-circle"
            ></i>
          </div>
        </div>
        <div class="py-2 pl-3 pr-2 col-lg-8">
          <div>
            ...
            <div v-show="!loading">
              <small v-show="valid" class="text-success"
                >{{
                  $t(
                    "Verified file is valid",
                    "The file {filename} is valid.",
                    { filename: file?.name }
                  )
                }}
              </small>
              <small v-show="invalid" class="text-danger"
                >{{
                  $t(
                    "Verified file revoked",
                    "The file {filename} has been revoked.",
              ...
              <small v-show="notFound" class="text-danger"
                >{{
                  $t(
                    "Verified file is invalid",
                    "The file {filename} is invalid.",
          ...
          <div v-show="valid">
            ...
            <a
              class="break-word font-weight-bold small"
              :title="creator"
              target="_blank"
              :href="'https://' + network + 'etherscan.io/address/' + creator"
              v-if="contract"
          ...
          <small class="text-hint" v-show="errorValidating">
            {{
              $t(
                "Error while verifying file",
                "File could not been verified due to technical problems. Please try again."
              )
            }}
          </small>
          ...
          <small v-show="showDetails" class="text-hint text-left">
            ...
            <div class="container mt-2">
              ...
              <div v-show="valid && hash" class="row mb-2">
                ...
                <div class="col-lg-8 pl-0 break-word">
                  {{ hash | addressOrHash }}
                </div>
              </div>
              <div class="row mb-2" v-show="contract">
                <div class="col-lg-4 pl-0 pr-0 text-hint">
                  {{ $t("Verified file contract addr", "Contract Address:") }}
                </div>
              ...
              <div class="row" v-show="tx">
                <div class="col-lg-4 pr-0 pl-0 text-hint">
                  {{ $t("Verified file transaction", "Transaction:") }}
                </div>
                <div class="col-lg-8 pl-0">
                  <a
...
export default {
  ...
  data() {
    return {
      ...
      hash: undefined,
      network:
        process.env.VUE_APP_BLOCKCHAIN_NET === "sepolia" ? "sepolia." : "",
      timestamp: undefined,
      creator: undefined,
      signatures: [],
      contract: undefined,
  ...
  methods: {
    async verify() {
      ...
      try {
        ...
        const block = await this.wallet().web3.eth.getBlock(
          transaction.blockNumber
        );
        // *1000 is conversion to seconds
        this.timestamp = new Date(block.timestamp * 1000).toUTCString();

        const signersArr = await this.wallet()
          .proxeusFS.contract.methods.getFileSigners(this.hash)
          .call();
        Promise.all(
          signersArr.map(async (signerAddr) => {
            const registrationTxBlock =
              await this.wallet().getRegistrationTxBlock(signerAddr);
            return {
              address: signerAddr,
              txHash: registrationTxBlock.txHash,
              block: block,
              time: new Date(
                registrationTxBlock.block.timestamp * 1000
              ).toUTCString(),
            };
          })
        ).then((sn) => {
