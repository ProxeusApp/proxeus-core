<template>
  ...
  >
      ...
      <form v-show="!done" @submit.prevent="changeEmailRequest">
        ...
        <div>
          <span class="text-muted" style="white-space: normal">{{
            $t(
              "New email explanation",
              "Provide a valid email and proceed by clicking the button below to receive a link."
            )
          }}</span>
        </div>
        <table style="width: 100%">
          <tr>
            <td style="text-align: right">
              <button class="btn btn-primary px-3 mt-1" type="submit">
                {{ $t("Validate new email", "Validate") }}
              </button>
            </td>
          </tr>
        </table>
    ...
    <animated-input
      ...
      v-model="me.etherPK"
      :action="{
        Name: $t('inline change badge', 'change'),
        Func: updateEthereumAddress,
      }"
    />
    <span v-show="walletErrorMessage" class="error">{{
      walletErrorMessage
    }}</span>
    <br />
    ...
    <span class="text-muted" style="white-space: normal">{{
      $t(
        "Want to be found explanation",
        "Uncheck this property if you want to be found only by your blockchain address."
      )
    }}</span>
    <br /><br />
    <div class="sub-title">{{ $t("Delete account") }}</div>
    <div v-if="showConfirm">
      <button @click="showConfirm = false" class="btn btn-secondary mt-1 ml-1">
        {{ $t("Cancel") }}
      </button>
      <button
        class="btn btn-danger mt-1 float-right mr-1"
        @click="deleteAccount"
      >
    ...
    <div class="text-muted mt-1 ml-1" style="white-space: normal">
      {{ $t("Account deletion can not be undone.") }}
    </div>
    <span v-show="deleteErrorMessage" class="error">{{
      deleteErrorMessage
    }}</span>
    <api-key :user="me" />
  ...
  methods: {
    async deleteAccount() {
      try {
        ...
        this.deleteErrorMessage = this.$t(
          "There was en error deleting your account. Please try later or contact support."
        );
      }
    },
    ...
    async metamaskLogin() {
      ...
      if (this.account === undefined) {
        this.walletErrorMessage = this.$t(
      ...
      this.wallet
        .signMessage(this.challenge, this.account)
        .then((signature) => {
          $(this.$refs.inputBcAddr.$refs.field).cleanFieldErrors();
          axios.post("/api/change/bcaddress", { signature }).then(
            (res) => {
              this.challenge = null;
            ...
            (err) => {
              ...
              if (err.response && err.response.status === 422) {
                $(this.$refs.inputBcAddr.$refs.field).showFieldErrors({
    ...
    changeEmailRequest() {
      axios.post("/api/change/email", { email: this.email }).then(
        (res) => {
          this.cleanErr();
          this.done = true;
        },
        (err) => {
          ...
          this.app.handleError(err);
          if (err.response && err.response.status === 422) {
            $(this.$refs.inputEmail.$refs.field).showFieldErrors({
              errors: err.response.data,
            });
          } else {
            this.$notify({
              ...
              text: this.$t(
                "There was an error changing the email address. Please try again or if the error persists contact the platform operator."
              ),
              type: "warning",
            });
          }
    ...
    save() {
      ...
      axios.post("/api/me", this.me).then(
        (response) => {
          this.pass = "";
        ...
        (err) => {
          ...
          this.$notify({
            ...
            title: this.$t("Error"),
            text: this.$t(
              "There was an error saving the changes. Please try again or if the error persists contact the platform operator."
            ),
            type: "error",
          });
        }
