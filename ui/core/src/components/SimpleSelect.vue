<template>
  <div :tabindex="disabled?false:'0'" ref="myroot" class="ss-sel-main">
        ...
        <tr>
          <td>
            <span class="ss-selected">
                <span ref="selected" class="sss" style="vertical-align: middle;"></span>
            </span>
          </td>
          <td class="min">
    ...
    <div class="ss-list" v-show="shouldShowDropDown()">
      <div ref="unselect" v-if="unselect" @click="unselectClick" class="ss-unselect" v-html="unselectedHTML"></div>
      <ul ref="items">
        <li tabindex="-1" v-for="(option, id) in options" :key="get(option,idProp)||id"
            class="cursor-pointer outline-none" @click.prevent="select($event, option, id)">
          <slot :option="option">
            <span v-if="typeof option === 'string'">{{ option }}</span>
            <span v-else-if="get(option, labelProp)">{{ get(option, labelProp) }}</span>
            <span v-else>your option value: {{JSON.stringify(option)}}, please provide labelProp</span>
          </slot>
        </li>
      </ul>
    </div>
    <div class="errors"></div>
    <div v-show="disableWithVisibleLayer && disabled" class="ss-sel-disabled"></div>
  </div>
</template>
    ...
    if (this.options && this.options.length > 0) {
      ...
      for (let i = 0; i < this.options.length; i++) {
        ...
        if (maybeSelected === this.value || maybeSelected === selectedVal) {
          ...
          this.lastSelectedLi = this.lastTargetLi = this.$refs.items.children[i]
          this.setActive(this.lastSelectedLi)
          this.renderSelected()
          return
        }
      }
      if ((selectedVal === null || selectedVal === undefined || selectedVal === '') && !isNaN(this.value)) {
  ...
  methods: {
    ...
    getTargetLI (noDefault) {
      ...
      if (!noDefault &&
        ...
        this.selectedIndex = 0
        this.selectedKey = this.get(this.options[this.selectedIndex], this.idProp)
    ...
    renderSelected () {
      ...
      if (this.renderNow && !this.show && this.selected && typeof this.selected === 'function') {
      ...
      if (this.selectedIndex > -1 &&
        ...
        this.$refs.items.children.length === this.options.length &&
        this.selectedIndex < this.options.length) {
        if (this.lastOptionsLength !== this.options.length) { // check if the key can be found after the options changed
          let foundItAgainObj
          for (let i = 0; i < this.$refs.items.children.length; i++) {
          ...
          if (!foundItAgainObj) { // key does not exist in the new options.. unselect
            this.resetAll()
          }
          this.lastOptionsLength = this.options.length
          return
    ...
    justHide () {
      this.show = false
      if (this.$refs.items && this.$refs.items.children && this.$refs.items.children.length) {
        for (let i = 0; i < this.$refs.items.children.length; i++) {
          this.removeTargetClass(this.$refs.items.children[i])
        }
      }
...
<style lang="scss">
  ...
  .ss-sel-main .ss-list li:hover, .ss-sel-main .ss-unselect:hover, .ss-sel-main .ss-list .litarget {
    background: #0000000d;
  }
