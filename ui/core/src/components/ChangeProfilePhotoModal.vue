<template>
<b-modal class="profilephoto-modal b-modal" v-model="show"
  ...
  <button v-show="file" @click="onDialogOk" class="btn btn-primary">{{$t('Set as profile photo')}}</button>
  <button @click="onDialogHide" class="btn btn-secondary">{{$t('Cancel')}}</button>
  </template>
...
export default {
  ...
  methods: {
    ...
    cropped (file, croppedBlob) {
      axios.post('/api/my/profile/photo', croppedBlob, {
        headers: {
