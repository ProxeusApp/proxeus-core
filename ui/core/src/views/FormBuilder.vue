<template>
  <div class="">
    <vue-headful
      :title="
        $t('FormBuilder title prefix', 'Proxeus - ') +
        ((form && form.name) || $t('Formbuilder title', 'Formbuilder'))
      "
      v-if="!userSrc"
    />
    <top-nav
      :title="form.name || $t('Formbuilder title', 'Formbuilder')"
      :sm="true"
      v-if="modalMode === false && form"
      class="mb-0"
      :returnToRoute="{ name: 'Forms' }"
    >
      <button
        slot="buttons"
        v-if="form && app.userIsUserOrHigher()"
        class="btn btn-link"
      ...
      <button
        slot="buttons"
        v-if="app.amIWriteGrantedFor(form)"
        class="btn btn-link"
      ...
      <button
        ...
        @click="save"
        :disabled="app.amIWriteGrantedFor(form) === false"
      >
        Save
      </button>
    </top-nav>
    <div class="container-fluid">
      <div
        class="toggle-row row py-3 position-absolute"
        v-if="form"
        v-show="infoToggled"
      >
      ...
      <form
        ...
        class="app-formbuilder row h-100"
      >
        <div class="dynamic-top col-sm-12 p-layout-fixed-scroll-container">
          <div
            ...
            v-show="infoToggled"
          ></div>
          <div id="formBuilderMainTag" class="row"></div>
        </div>
      </form>
    </div>
    <permission-dialog
      v-if="form"
      :save="save"
      :publicLink="app.makeURL('/p/form/' + form.id)"
      v-model="form"
      :setup="setupDialog"
    />
  </div>
...
export default {
  ...
  created() {
    ...
    if (this.userSrc) {
      ...
      axios.get("/api/admin/form/" + this.id).then(
        (response) => {
          ...
          if (response.data && response.data.id) {
            ...
            this.$notify({
              ...
              title: this.$t("Error"),
              text: this.$t(
                "Could not load form. Please try again or if the error persists contact the platform operator."
              ),
              type: "error",
            });
    ...
    initFb(store) {
      ...
      jQuery(function () {
        const myHtmlEntry = function (id, langCode, text) {
          return (
            '<div class="fb-i18n-entry"><p class="fb-i18n-key">' +
            id +
            '</p><span class="fb-i18n-lang">' +
            langCode +
            '</span><p class="fb-i18n-text">' +
            text +
            "</p></div>"
          );
        };
        ...
        const formBuilderOptions = {
          ...
          test: {
            ...
            onReset: function () {
              $.get(
                "/api/admin/form/test/data/" + _this.id + "?reset=true",
                function (data) {
          ...
          i18n: {
            ...
            onSearch: function (text, callback) {
              $.get("/api/admin/i18n/search?c=" + text)
                .done(function (data) {
                  const dataList = [];
                  if (data) {
                    ...
                    for (id in data) {
                      dataIDObj = data[id];
                      const hasDataProp = Object.prototype.hasOwnProperty.call(
                        data,
                        id
                      );
            ...
            onDisplay: function (data) {
              const hasDatai18n = Object.prototype.hasOwnProperty.call(
                data,
                "i18n"
              );
          ...
          component: {
            ...
            searchComp: function (text, callback) {
              const getOpts = {
                ...
                url:
                  "/api/admin/form/component?l=1000" +
                  (text ? "&c=" + text : ""),
                success: function (data) {
...
<style lang="scss">
@import "../assets/styles/legacy/backend.scss";
@import "../assets/styles/legacy/formbuilder.scss";
