...
<div>
  ...
  <vue-headful :title="$t('Sign in title', 'Proxeus - Sign in')"/>
    ...
    <div class="row">
      <div class="col-6 mt-3 text-center d-flex align-items-center border-right" v-if="app.wallet && metamaskLoginAvailable">
        <div class="mid align-self-center w-100">
          ...
          <p class="light-text">{{$t('Use your MetaMask Wallet to sign in.')}}</p>
          ...
          <button class="btn btn-primary px-3" @click="loginWithSignature"
                  v-if="metamaskLoginAvailable">{{$t('Sign in with Metamask')}}
          </button>
        </div>
      </div>
        ...
        <form class="text-center" @submit.prevent="login">
          ...
          <div class="form-group">
            ...
            <router-link :to="{path:'/register'}" class="" data-toggle="tooltip"
                         data-boundary="window" :title="$t('Sign up')">
              {{$t('Sign up')}}
            </router-link>
          </div>
          <button class="btn btn-primary px-3" type="submit">{{$t('Sign in')}}</button>
        </form>
      </div>
    </div>
  </div>
