<template>
  <div>
    ...
    <vue-headful
      :title="$t('Reset password title', 'Proxeus - Reset Password')"
    />
          ...
          <form v-show="!done" class="text-center" @submit.prevent="reset">
            ...
            <span class="text-muted" style="display: inline-block">{{
              $t(
                ...
                "Reset the password by clicking the button below."
              )
            }}</span>
            <div style="width: 100%">
              <button class="btn btn-primary px-3 mt-3" type="submit">
                {{ $t("Reset") }}
              </button>
            </div>
          </form>
          <div v-show="done">
            <div class="my-3">{{ $t("Password reset succeeded.") }}</div>
...
export default {
  ...
  methods: {
    ...
    reset() {
      axios
        .post("/api/reset/password/" + this.token, { password: this.password })
        .then(
          (res) => {
            this.cleanErr();
            this.done = true;
          ...
          (err) => {
            ...
            if (err.response && err.response.status === 422) {
              $(this.$refs.inputPassword).showFieldErrors({
              ...
              $(this.$refs.inputPassword).showFieldErrors({
                errors: {
                  password: [
                    { msg: this.$t("Please request a password reset.") },
                  ],
                },
              });
