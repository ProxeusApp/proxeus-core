      ...
      <div class="col-12">
        <form v-show="!done" class="text-center" @submit.prevent="reset">
          ...
          <div v-show="pwResetRequest" class="form-group">
            <a href="/reset/password" class="">{{$t('Request password reset')}}</a>
          </div>
          <span class="text-muted"
                style="display: inline-block;">{{$t('Password reset explanation', 'Reset the password by clicking the button below.')}}</span>
          <div style="width:100%;">
            <button class="btn btn-primary px-3 mt-3" type="submit">{{$t('Reset')}}</button>
          </div>
        </form>
        <div v-show="done">
...
export default {
  ...
  methods: {
    ...
    reset () {
      axios.post('/api/reset/password/' + this.token, { password: this.password }).then(res => {
        this.cleanErr()
        this.done = true
      }, (err) => {
        this.cleanErr()
