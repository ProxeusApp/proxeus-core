  ...
  <div>
    ...
    <div class="main-container">
      ...
      >
        <div ref="inviteCont" class="form-group" style="text-align: left">
          ...
          <small class="text-muted">{{
            $t("Enter the email of the user you would like to invite.")
          }}</small>
          <div class="form-group">
            ...
            <small class="text-muted"
              >{{
                $t(
                  "Select the role the user is going to get, in case the invitation is going to be accepted."
                )
              }}
            </small>
          </div>
      ...
      >
        ...
        <div
          slot="newItemForm"
          class="pb-3 new-item-form list-group-item-action bg-light container-fluid"
          v-show="newItemFormVisible"
          style="position: relative"
        >
          <i
            @click="newItemFormVisible = false"
          ...
          <div class="row">
            <div class="col-sm-12">
              <div class="form-group">
                ...
                <input
                  type="text"
                  v-model.trim="newElement.etherPK"
                  name="newElementEtherAddress"
                  placeholder="0x245F..."
                  id="etherAddressInput"
                  class="form-control"
                  required
                />
                <small class="light-text"
                  >The users ethereum account address</small
                >
              </div>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    onDialogOk() {
      axios.post("/api/admin/invite", this.invite).then(
        (res) => {
          this.cleanErr();
          this.modalShow = false;
          this.invite = { role: 1 };
        ...
        (err) => {
          ...
          if (err.response && err.response.status === 422) {
            $(this.$refs.inviteCont).showFieldErrors({
    ...
    createUser() {
      ...
      this.newElement.id = "";
      axios.post("/api/admin/user/update", this.newElement).then(
        (response) => {
          if (response.data.id) {
