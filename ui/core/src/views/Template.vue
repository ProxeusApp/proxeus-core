<template>
  <div class="h-100 TplIde">
    <vue-headful
      :title="
        $t('Template title prefix', 'Proxeus - ') +
        ((template && template.name) || $t('Template title', 'Template'))
      "
    />

    <top-nav
      :title="template && template.name ? template.name : $t('Template')"
      :sm="true"
    ...
    >
      <template slot="buttons" v-if="hasRenderedDocument">
        <div v-for="(type, i) in downloadButtons" :key="i">
          <a
            :href="`/api/admin/template/ide/download/${id}?format=${type}`"
            class="filedownload-btn px-2"
            target="_blank"
          >
            <table style="text-align: center">
              <tr>
                <td>
                  <span class="d-block text-uppercase">{{ type }}</span>
                </td>
              </tr>
              <tr>
                <td>
      ...
      <button
        slot="buttons"
        v-if="template && app.userIsUserOrHigher()"
        type="button"
        class="btn btn-link"
      ...
      <button
        type="button"
        v-if="template && app.amIWriteGrantedFor(template)"
        slot="buttons"
        class="btn btn-link"
    ...
    <div class="container-fluid layout-fixed-scroll-container">
      ...
      <div class="row h-100 align-items-start">
        <div
          :class="{
            'col-6 layout-fixed-scroll-view': previewVisible,
            'col-12': previewVisible === false,
          }"
        >
          ...
          <div class="row mt-3" v-if="meta !== null && meta && template">
            <div class="col" v-for="lang in meta.activeLangs" :key="lang.Code">
              <document-template-chooser
                :readOnly="template && !app.amIWriteGrantedFor(template)"
                :lang="lang"
        ...
        <div
          class="col-6 px-0 layout-fixed-scroll-view right-0"
          :class="{ overflowHidden: loadingPdf }"
        ...
        <div id="libreAssistanceContainer"></div>
      </div>
    </div>
    <permission-dialog
      v-if="template"
      :save="save"
      :publicLink="app.makeURL('/p/template/' + template.id)"
      v-model="template"
      :setup="setupDialog"
    />
  </div>
...
export default {
  ...
  methods: {
        ...
        this.loadTemplate(
          (resp) => {
            if (resp.data) {
              ...
              axios
                .post(
                  `/api/admin/template/update?id=${this.template.id}`,
                  this.template
                )
                .then(
                  (response) => {
    ...
    loadForms(searchTerm) {
      const listUrl =
        "/api/admin/template/ide/form" + (searchTerm ? "?c=" + searchTerm : "");
    ...
    loadComponents() {
      axios.get("/api/admin/form/component?l=1000").then(
        (res) => {
    ...
    loadLanguageMeta() {
      axios.get("/api/admin/i18n/meta").then(
        (response) => {
    ...
    loadTemplate(success, error) {
      axios.get("/api/admin/template/" + this.id).then(
        (response) => {
    ...
    downloadIdeFile(noError) {
      ...
      axios
        .get("/api/admin/template/ide/download/" + this.id, {
          responseType: "blob",
          ...
          (err) => {
            ...
            if (axios.isCancel(err)) {
              ...
              if (noError === undefined) {
                this.notifyError(
                  this.$t("Could not render PDF. Please try again.")
                );
              }
            }
          }
    ...
    setServerFileActive(lang) {
      ...
      if (this.lastActiveLang !== lang.Code) {
        axios
          .get(`/api/admin/template/ide/active/${this.id}/${lang.Code}`)
          .then(
            (res) => {
            ...
            (err) => {
              ...
              this.loadingPdf = false;
              this.notifyError(
                this.$t("Could not connect to the server. Please try again.")
              );
            }
          );
      } else {
