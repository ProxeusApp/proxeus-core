<template>
  <responsive-sidebar
    class="sidebar-app w-100 sidebar d-flex flex-column flex-shrink-0"
    :toggled="toggled"
    v-if="app.userIsCreatorOrHigher()"
  >
    <div class="pb-4">
      <router-link
        :to="{ name: 'Workflows' }"
        class="navbar-brand mb-0 w-100 px-3 mx-0"
      >
        <svg
          width="125"
          viewBox="0 0 187 55"
    ...
    <nav class="nav main-nav flex-column main-nav-backend">
      <li class="nav-item" v-if="app.userIsCreatorOrHigher()">
        <router-link
          ...
          :title="$t('Menu Workflows', 'Workflows')"
          ><span class="material-icons mdi mdi-source-branch"></span
          ><span class="nav-link-title">{{
            $t("Menu Workflows", "Workflows")
          }}</span>
        </router-link>
      </li>
      <li class="nav-item" v-if="app.userIsCreatorOrHigher()">
        <router-link
          ...
          data-placement="right"
          data-boundary="window"
          :title="$t('Menu Templates', 'Templates')"
          ><span class="material-icons mdi mdi-code-block-tags"></span
          ><span class="nav-link-title">{{
      ...
      <li class="nav-item" v-if="app.userIsCreatorOrHigher()">
        <router-link
          ...
          data-placement="right"
          data-boundary="window"
          :title="$t('Menu Forms', 'Forms')"
          ><span class="material-icons">view_quilt</span
          ><span class="nav-link-title">{{ $t("Menu Forms", "Forms") }}</span>
        </router-link>
      </li>
      ...
      <li class="nav-item" v-if="superadmin">
        <router-link
          ...
          data-placement="right"
          data-boundary="window"
          :title="$t('Menu Internationalization', 'Internationalization')"
        >
          <span class="material-icons">language</span
          ><span class="nav-link-title">{{
            $t("Menu Internationalization", "Internationalization")
          }}</span>
        </router-link>
      </li>
      <li class="nav-item" v-if="superadmin">
        <router-link
          :to="{ name: 'AdminImportExport' }"
          class="nav-link"
          data-toggle="tooltip"
          data-placement="right"
          data-boundary="window"
      ...
      <li class="nav-item" v-if="app.userIsRoot()">
        <router-link
          ...
          data-placement="right"
          data-boundary="window"
          :title="$t('Menu System Settings', 'System Settings')"
        >
          <span class="material-icons">settings_applications</span
          ><span class="nav-link-title">{{
            $t("Menu System Settings", "System Settings")
          }}</span>
        </router-link>
      </li>
    </nav>
      ...
      <ul class="nav secondary-nav flex-column">
        <li class="nav-item">
          <router-link
            ...
            data-placement="right"
            data-boundary="window"
            :title="$t('Menu Help / Support', 'Help / Support')"
            ><span class="mdi mdi-help-circle-outline material-icons"></span
            ><span class="nav-link-title">{{
              $t("Menu Help / Support", "Help / Support")
            }}</span>
          </router-link>
        </li>
        <li class="nav-item" v-if="app.userIsUserOrHigher()">
          <a
            ...
            data-placement="right"
            data-boundary="window"
            :title="$t('Menu Frontend', 'User View')"
            ><span class="material-icons">event_note</span
            ><span class="nav-link-title">{{
              $t("Menu Frontend", "User View")
            }}</span>
          </a>
        </li>
      </ul>
