  ...
  <table style="height: 100%;width: 100%;">
    <tr>
      <td valign="top" style="vertical-align:top">
        <a class="navbar-brand mb-1 w-100 px-2 pt-2 pb-0 mx-0" href="#">
          ...
          <nav class="nav main-nav flex-column">
            ...
            <li class="nav-item">
              <router-link :to="{name:'SignatureRequests'}" class="nav-link" data-toggle="tooltip"
                ...
                class="nav-link-title">{{$t('Menu Signature Requests','Signature Requests')}} ({{ sigcount }})</span>
              </router-link>
            </li>
            <!--<li class="nav-item">-->
            <!--<router-link :to="{name:'ManagementList'}" class="nav-link" data-toggle="tooltip" data-placement="right"-->
            <!--data-boundary="window" title="Managment List"><span-->
            <!--class="material-icons mdi mdi-clipboard-text"></span><span-->
            <!--class="nav-link-title">Management List</span>-->
            <!--</router-link>-->
            <!--</li>-->
          </nav>
        </nav>
      ...
      axios.get('/api/user/document/signingRequests').then(async response => {
        if (response.data) {
          ...
          for (let i = 0, len = this.documents.length; i < len; i++) {
            this.getSigners(this.documents[i].hash).then(response => {
              if (!response.includes(this.me.etherPK)) {
