<template>
  <div class="app-workflow">
    <vue-headful
      :title="
        $t('Workflow title prefix', 'Proxeus - ') +
        ((workflow && workflow.name) || $t('Workflow title', 'Workflow'))
      "
    />
    <top-nav
      :title="workflow && workflow.name ? workflow.name : 'Workflow'"
      :returnToRoute="{ name: 'Workflows' }"
    >
      <button
        slot="buttons"
        v-if="workflow && app.userIsUserOrHigher()"
        class="btn btn-link"
      ...
      <button
        slot="buttons"
        v-if="workflow && app.amIWriteGrantedFor(workflow)"
        class="btn btn-link"
        @click="publishModalShow = true"
      >
        <span>{{ $t("Publish") }}</span>
      </button>
      <button
        slot="buttons"
        v-if="app.userIsUserOrHigher()"
        @click="
          app.exportData(
            '&id=' + id,
            null,
      ...
      <button
        ...
        @click="save"
        :disabled="app.amIWriteGrantedFor(workflow) === false"
      >
        Save
      </button>
    </top-nav>
    <div class="container-fluid p-0">
        ...
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            <div class="modal-header">
              ...
              <button
                @click="togglePriceErrorModal(false)"
                type="button"
                class="close"
                data-dismiss="modal"
                aria-label="Close"
            ...
            <div class="modal-body">
              <p>
                {{
                  $t(
                    "Before setting a workflow price make sure to set your ethereum address in your account settings on the top right."
                  )
                }}
              </p>
            </div>
      ...
      <form id="inputFormId" novalidate="novalidate" class="h-100">
        <div
          class="toggle-row row py-2 position-absolute"
          v-show="infoToggled"
          v-if="workflow"
        >
        ...
        <div class="workflow-wrapper">
          <div class="dynamic-top h-100">
            <div class="flow-chart-main">
              <textarea
                class="flow-chart-data"
                name="data.flow"
                style="display: none"
              ></textarea>
              <textarea
                class="progress-flow"
                name="data.progressFlow"
                style="display: none"
              ></textarea>
              <div class="wfchart-main">
                <div class="fcf-main" style="width: 100%">
                  <div class="flow-chart-finder">
                    <div>
                          ...
                          <form autocomplete="off">
                            <div class="input-group w-100 search-box">
                              ...
                              <input
                                type="search"
                                autocomplete="off"
                                ref="workflowElementAdd"
                                class="form-control text-field flow-chart-finder-input"
                                :placeholder="
                                  $t('Search for workflow elements…')
                                "
                                style="box-sizing: border-box"
                              />
                              <div
                                class="input-group-append fcf-layout-switch p-1 bg-light border"
                              >
                                <button
                                  type="button"
                      ...
                      <div
                        style="display: none"
                        class="flow-chart-finder-result-main"
                      >
                ...
                <div class="wfchart-snavi">
                  <div class="wfc-snn">
                    ...
                    <div
                      data-id="condition"
                      class="flow-chart-node flow-chart-node-condition"
                    >
                      <div class="flow-chart-node-inner">
                        <div class="fci-node">
                          <i
                            class="node-icon fcn-condition mdi mdi-rhombus-outline"
                            aria-hidden="true"
                          ></i>
                          <div class="flow-chart-finder-simple">
                            {{ $t("condition") }}
                          </div>
                        </div>
                      </div>
                    ...
                    <div
                      data-id="placeholder"
                      class="flow-chart-node flow-chart-node-placeholder"
                    >
                      <div class="flow-chart-node-inner">
                        <div class="fci-node">
                          <i
                            class="node-icon fcn-placeholder mdi mdi-hexagon"
                            aria-hidden="true"
                          ></i>
                          <div class="flow-chart-finder-simple">
                            {{ $t("placeholder") }}
            ...
            <ul class="custom-menu">
              <li data-action="restrictAccess" for-nodes="template">
                Restrict Access
              </li>
              <li data-action="contextDefineUser" for-nodes="contextSwitch">
                Define new user
              </li>
              <li data-action="connectorFunc" for-nodes="connector">
          ...
          <div class="modal-content">
            ...
            <div class="modal-body">
              <form novalidate="novalidate">
                ...
                <div class="row">
                  <div class="col-md-6">
                    <div class="field-parent">
                      <label for="cond_name" class="ft-label">{{
                        $t("Name")
                      }}</label>
                      <input
                        type="text"
                  ...
                  <div class="col-md-6">
                    <div class="field-parent">
                      <label for="cond_descr" class="ft-label">{{
                        $t("Description")
                      }}</label>
                      <textarea
            ...
            <div class="modal-footer">
              <div class="row">
                <div class="col-md-12">
                  <label
                    for="condition_cases_parser"
                    style="margin-right: 10px"
                  >
                    {{ $t("auto change cases by parsing the script") }}
                    <input
                      id="condition_cases_parser"
                      type="checkbox"
                      checked
                    />
                  </label>
                  <button
                    type="button"
                    class="btn btn-primary"
                    @click="wfm.conditionDialogHandler.save(this)"
                  >
                    {{ $t("Save") }}
                  </button>
                </div>
            ...
            <div class="modal-body">
              <form novalidate="novalidate">
                ...
                <div class="row">
                  <div class="col-md-12">
                    <div class="field-parent">
                      <label for="placeholder_description" class="ft-label">{{
                        $t("Description")
                      }}</label>
                      <textarea
                        :placeholder="$t('Description')"
              ...
              <div class="row">
                <div class="col-md-12">
                  <button
                    type="button"
                    class="btn btn-primary"
                    @click="wfm.placeholderDialogHandler.save(this)"
                  >
                    {{ $t("Save") }}
                  </button>
                </div>
    ...
    >
      <div class="d-block">
        {{ $t("If you publish this workflow anyone can start this workflow.") }}
      </div>
      <div class="d-block mt-3">
        {{ $t("Are you sure you want to publish this workflow?") }}
      </div>
      <div class="d-block bold bg-primary"></div>
    </b-modal>

    <permission-dialog
      v-if="workflow"
      :save="save"
      :publicLink="app.makeURL('/p/workflow/' + id)"
      :publicLink2="app.makeURL('/document/' + id)"
      v-model="workflow"
      :setup="setupDialog"
    />
    <list-item-dialog
      ...
      :setup="setupPublishResponseDialog"
      :timestamp="false"
      :noBtns="true"
    >
      ...
      <div class="d-block">
        {{
          $t(
            "Please review the following entities carefully in order to publish."
          )
        }}
      </div>
    </list-item-dialog>
...
export default {
  ...
  mounted() {
    ...
    axios.get("/api/admin/workflow/" + this.id).then(
      (response) => {
        if (!response.data) {
          this.workflow = {};
        } else {
      ...
      (error) => {
        ...
        if (error.response && error.response.status === 404) {
          ...
          this.$notify({
            ...
            title: this.$t("Error"),
            text: this.$t(
              "Could not load Workflow. Please try again or if the error persists contact the platform operator.\n"
            ),
            type: "error",
          });
  ...
  methods: {
    ...
    publish() {
      ...
      axios
        .post("/api/admin/workflow/update?publish&id=" + this.id, workflowClone)
        .then(
          (response) => {
            this.snapshot(this.workflow, this.skipFromSnapshot);
                    ...
                    if (item) {
                      item.getLink = function () {
                        if (
                          this.id &&
                          this.type &&
                          /workflow|form|template/.test(this.type)
                        ) {
          ...
          (err) => {
            ...
            this.$notify({
              ...
              title: this.$t("Error"),
              text: this.$t(
                "Could not publish workflow. Please try again or if the error persists contact the platform operator.\n"
              ),
              type: "error",
            });
          }
    ...
    save() {
      ...
      axios
        .post("/api/admin/workflow/update?id=" + this.id, workflowClone)
        .then(
          (response) => {
            this.snapshot(this.workflow, this.skipFromSnapshot);
            this.$notify({
          ...
          (err) => {
            ...
            if (err.response.data === "can not set price without eth addr") {
              ...
              this.$notify({
                ...
                title: this.$t("Error"),
                text: this.$t(
                  "Could not save workflow. Please try again or if the error persists contact the platform operator.\n"
                ),
                type: "error",
              });
            }
        ...
        initConditionDialogHandler: function () {
          this.conditionDialogHandler = {
            ...
            open: function (node, callback, isNewNode) {
              ...
              if (this.init) {
                this.$autoParseCases = this.$dialog.find(
                  "#condition_cases_parser"
                );
    ...
    "value": "something else"
            ...
            save: function (el) {
              try {
                ...
                if (this.$autoParseCases.is(":checked")) {
                  this.currentNode._cases = parseFunctionCases(
                    js,
                    this.casesEditor.getValue()
                  );
                } else {
                  try {
                    this.currentNode._cases = JSON.parse(
                      this.casesEditor.getValue()
                    );
                  } catch (eee) {
                ...
                if (this.currentNode._cases) {
                  this.casesEditor
                    .getSession()
                    .setValue(
                      JSON.stringify(this.currentNode._cases, null, "\t"),
                      -1
                    );
                } else {
                  this.casesEditor.getSession().setValue("", -1);
                ...
                this.currentNode.label = this.$dialog
                  .find("[name='condition.name']")
                  .val();
                this.currentNode.name = this.currentNode.label;
                this.currentNode.detail = this.$dialog
                  .find("[name='condition.detail']")
                  .val();
        ...
        initPlaceholderDialogHandler: function () {
          this.placeholderDialogHandler = {
            ...
            save: function () {
              try {
                const detail = this.$dialog
                  .find("[name='placeholder.detail']")
                  .val();
        ...
        main: function () {
          ...
          var _ = this;
          this.$finderResultMain = this.$main.find(
            ".flow-chart-finder-result-main"
          );
          this.$finderResult = this.$finderResultMain.find(
            ".flow-chart-finder-cnt"
          );
          this.$finderResultBody = this.$finderResult.find(".fcfr-body");

          this.$finderResultCloseBtn =
            this.$finderResultMain.find(".fcfr-close");
          ...
          var gridBtn = this.$main.find(
            ".flow-chart-finder .fcf-layout-switch .grid-layout-btn"
          );
          var rowBtn = this.$main.find(
            ".flow-chart-finder .fcf-layout-switch .row-layout-btn"
          );
          ...
          try {
            _.localLayoutSettings = $.parseJSON(
              Cookies.get(window.location.pathname)
            );
          } catch (e) {
          ...
          this.heightFixer = function () {
            if (_.$chartMain) {
              ...
              _.$chartMain.css(
                "height",
                $(document.body).height() - minusHeight - 2 + "px"
              );
            }
          };
          this.heightFixer();
        ...
        searchNow: function (v) {
          ...
          if (
            _.lastSearchText === v &&
            _.deletedNodesAfterLastSearch() &&
            _.lastSearchUnderThreeMinutes() &&
        ...
        showLegend: function () {
          if (!this.legendVisible) {
            ...
            if (!this.$legend) {
              ...
              this.$legend = $(
                '<div class="fcl fcl-legend flow-chart-finder-cnt fcf-row" style="display:none;">' +
                  rows +
                  "</div>"
              );
        ...
        storeSettings: function () {
          try {
            Cookies.set(
              ...
              JSON.stringify(this.localLayoutSettings)
            );
        ...
        attachMouseOffsetWorkaroundListener: function () {
              ...
              if (
                ...
                if (_.cloned) {
                  ...
                  if (_.$newTarget.attr("data-id") === "condition") {
                    _.conditionDialogHandler.open(
                      $.extend(
                        {},
                        _.leftMenuNodes[_.$newTarget.attr("data-id")]
                      ),
                      function (n) {
                        n.cases = n._cases;
                        n.data = n._data;
                    ...
                    _.placeholderDialogHandler.open(
                      $.extend(
                        {},
                        _.leftMenuNodes[_.$newTarget.attr("data-id")]
                      ),
          ...
          this.$chartMain.on(
            "mouseover touchmove touchend",
            this.workaroundListener
          );
        },
        droppedOverDZ: false,
            ...
            .draggable({
              ...
              onstart: function (event) {
                ...
                if (event.interaction.startOffset) {
                  _.dim.handlePercentX =
                    event.interaction.startOffset.left + _.dim.ml;
                  _.dim.handlePercentX = (100 / _.dim.w) * _.dim.handlePercentX;
                  _.dim.handlePercentY =
                    event.interaction.startOffset.top + _.dim.mt;
              ...
              onmove: function (event, a, b, c, d) {
                ...
                _.tl.y += event.dy;
                const translatePos =
                  "translate(" + _.tl.x + "px, " + _.tl.y + "px)";
                _.$dragComp[0].style.webkitTransform = translatePos;
                _.$dragComp[0].setAttribute(
                  "style",
              ...
              onend: function (e, a, b, c, d) {
                ...
                var revert = function () {
                  ...
                  if (_.localLayoutSettings.layout === "row") {
                    _.$dragComp[0].style.webkitTransform =
                      _.$dragComp[0].style.transform = "translate(1px, -3px)";
                  } else {
                    _.$dragComp[0].style.webkitTransform =
                      _.$dragComp[0].style.transform = "translate(2px, 2px)";
                  }
        ...
        createFinderItem: function (d) {
          if (d && d.data && d.data.id) {
            ...
            var $elExists = this.$finderResultBody.find(
              'div[data-id="' + d.id + '"]'
            );
      ...
      (function () {
        ...
        myWFM.beforeShowFinder = function () {
          if (
            myWFM.$finderResultMain.find("i.fcn-usr, i.fcn-tmpl, i.fcn-wflow")
              .length === 0
          ) {
        ...
        myWFM.addNode = function (t, n, d) {
          ...
          var dummy = {
            i: myWFM.dummyAddNodeIndex,
            name: n,
            detail: d,
            kind: t,
          };
          dummy.data = {
            id: dummy.kind + dummy.i,
            name: dummy.name,
...
.bg-graphpaper {
  ...
  background-image: linear-gradient(rgba($line, $sublineAlpha) 1px, $trans 1px),
    /*sub horiz*/ linear-gradient($line 1px, $trans 1px),
    /*main horiz*/
      linear-gradient(90deg, rgba($line, $sublineAlpha) 1px, $trans 1px),
    /*sub vert*/ linear-gradient(90deg, rgba($line, $lineAlpha) 1px, $trans 1px),
    /*main vert*/
      linear-gradient(
        $trans 3px,
        $block 3px,
  ...
  background-size: $gridSizeOverSubdivisions $gridSizeOverSubdivisions,
    $gridSize $gridSize, $gridSizeOverSubdivisions $gridSizeOverSubdivisions,
