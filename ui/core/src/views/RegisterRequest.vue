<template>
  <div>
    ...
    <div class="login-form container-fluid px-4 pt-2 mt-3 bg-light">
      <div class="row">
        <div
          class="col-6 text-center d-flex align-items-center border-right"
        ...
        >
          <div class="mid align-self-center w-100">
            ...
            <p class="light-text">
              {{ $t("Use your MetaMask Wallet to sign up.") }}
            </p>
            ...
            <button
              ...
              @click="loginWithSignature"
              v-if="metamaskLoginAvailable"
            >
              {{ $t("Sign up with Metamask") }}
            </button>
          </div>
        </div>
        <div
          :class="{
            'col-6': app.wallet && metamaskLoginAvailable,
            'col-12': !(app.wallet && metamaskLoginAvailable),
          }"
        >
          <form v-show="!done" class="text-center" @submit.prevent="request">
            ...
            <span class="text-muted" style="display: inline-block">{{
              $t(
                "Sign up explanation",
                "Sign up by providing your email and clicking the button below."
              )
            }}</span>
            <button
          ...
          <div v-show="done">
            <div class="my-3">
              {{
                $t(
                  "Sign up email sent explanation",
                  "Email sent. Please check your emails and visit the provided link to proceed."
                )
              }}
            </div>
            ...
            <a href="/login" class="btn btn-primary" style="float: right">{{
              $t("Sign in")
            }}</a>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    request() {
      axios.post("/api/register", { email: this.email }).then(
        (res) => {
          this.cleanErr();
          this.done = true;
        },
        (err) => {
          ...
          if (err.response && err.response.status === 422) {
            $(this.$refs.inputEmail).showFieldErrors({
            ...
            this.$notify({
              ...
              text: this.$t(
                "There was an unexpected error. Please try again or if the error persists contact the platform operator."
              ),
              type: "warning",
            });
          }
              ...
              (res) => {
                ...
                if (res.status >= 200 && res.status <= 299) {
                  this.app.initUserHasSession(
                    res.data.location || "/admin/workflow"
                  );
                } else {
                  this.app.deleteUserHasSession();
