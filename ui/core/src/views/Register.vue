        ...
        <div class="col-12">
          <form v-show="!done" class="text-center" @submit.prevent="reset">
            ...
            <div v-show="pwResetRequest" class="form-group">
              <a href="/register" class="" id="signupcontent">{{
                $t("Sign up")
              }}</a>
            </div>
            <span class="text-muted" style="display: inline-block">{{
              $t(
                ...
                "Set a password by clicking the button below."
              )
            }}</span>
            <div style="width: 100%">
          ...
          <div v-show="done">
            <div class="my-3">
              {{ $t("Registration completed. You can sign in now.") }}
            </div>
...
export default {
  ...
  methods: {
    ...
    reset() {
      axios
        .post("/api/register/" + this.token, { password: this.password })
        .then(
          (res) => {
            this.cleanErr();
            this.done = true;
          ...
          (err) => {
            ...
            if (err.response && err.response.status === 422) {
              ...
              $(this.$refs.inputPassword).showFieldErrors({
                errors: {
                  password: [{ msg: this.$t("Please sign up first.") }],
                },
              });
