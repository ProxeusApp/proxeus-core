<template>
  <div class="document-view">
    <vue-headful
      :title="
        $t('Flow title prefix', 'Proxeus - ') +
        (name || $t('Document title', 'Document'))
      "
    />
    <top-nav
      ...
      class="border-bottom-0 bg-white"
    >
      <language-drop-down slot="buttons" style="margin-top: 3px" />
    </top-nav>
    <div
      class="container-fluid document-inner-view bg-light"
      v-if="status && name"
    >
      <div class="row h-100">
        <div class="col-md-3 mt-3">
          <ul class="nav mod_stepnav flex-column nav-pills">
            ...
            >
              <span
                class="btn-link nav-link text-truncate"
                :class="{
                  active:
            ...
            <li class="nav-item">
              <span
                class="btn-link nav-link active text-truncate"
        ...
        <div class="col-md-6 mt-3">
          <div
            class="document-form card bg-transparent w-100 border-0"
          ...
          >
              ...
              <div class="processing-hint">
                {{
                  $t(
                    ...
                    "Processing your request. This might take a while."
                  )
                }}
              </div>
            </div>
            <div>
              ...
              <div class="card-body card-form-body bg-white border-0"></div>
            </div>
            <div
              v-show="loading === false && isConfirmationStep === false"
              class="v-fade"
            ...
            >
              ...
              <div
                id="docForm"
                class="card-body card-form-body bg-white border-0"
              >
                <div
                  class="form form-compiled"
                  v-append.sync="formSrc"
                  @appended="appended"
                  v-show="isConfirmationStep === false"
                ></div>
              </div>
            </div>

            <div
              v-show="loading === false && isConfirmationStep"
              class="v-fade"
              style="position: absolute; top: 0; left: 0; bottom: 0; right: 0"
            >
              <div
                v-if="!isConnectedAccount"
                class="alert alert-warning mb-3"
                role="alert"
              >
                {{
                  $t(
                    "Document register help",
                    "Make sure that you are logged into the correct account in MetaMask. The Document-Registration must be made from the ethereum-account that you have connected in your account settings. Switch Metamask account and make sure that you have set your ethereum address in your account account settings on the top right."
                  )
                }}
              </div>
              <div class="card-header border-bottom-0">
              ...
              <div class="card-body card-form-body bg-white border-0">
                <div class="last-step">
                  <form v-on:submit.prevent="confirm">
                    <div
                      class="mb-2"
                      v-show="submitting === false && pricePerFile"
                    >
                      <div class="easy-read">
                        {{ pricePerFileFormatted }} per File.
                      </div>
                      <span class="light-text"
                        >(Ethereum transaction cost excluded)</span
                      >
                    </div>
                    <div class="mb-2" v-show="submitting === false">
                      <animated-input
                        label="Additional Text data"
                        :max="32"
                        v-model="data"
                      />
                      <span class="light-text"
                        >Information entered here will be publicly visible to
                        anyone.</span
                      >
                    </div>
                    <button
                      ...
                      :disabled="submitting || !isConnectedAccount"
                      v-show="submitting === false"
                    >
                      Submit
                    </button>
                    <spinner
                    ...
                    <div class="text-center">
                      <transition name="fade" mode="out-in" v-if="approvingXES">
                        <p class="text-primary px-5">
                          Please confirm the release of XES from your wallet in
                          MetaMask.
                        </p>
                      </transition>
                      <transition
                      ...
                      >
                        <p class="text-primary px-5">
                          Please confirm the registration of your document on
                          the blockchain in MetaMask.
                        </p>
                      </transition>
                      <p class="light-text" v-show="submitting">
                        If MetaMask doesn't pop up automatically, click on the
                        MetaMask icon in the top right corner of your browser.
                      </p>
                    </div>
                  </form>
                </div>
            ...
            <div class="card-footer bg-primary d-flex flex-row">
              <button
                type="submit"
                class="btn btn-primary border mr-auto"
                @click="previous"
                :disabled="loading"
              ...
              <button
                type="submit"
                class="btn btn-primary border ml-auto"
                @click="next"
                :disabled="loading"
                v-if="status.hasNext"
              >
              ...
              <button
                ...
                :disabled="loading"
                v-else-if="
                  status.hasNext === false && isConfirmationStep === false
                "
              >
                <i class="material-icons">chevron_right</i>
              </button>
        ...
        <div
          class="col-md-3 mt-3 document-scroll-view"
          :class="{ 'has-pdf': documentPreviews != null }"
        >
          <template v-if="documentPreviews">
            <div class="d-flex flex-row flex-wrap">
              <template v-for="doc in documentPreviews">
                <pdf-preview
                  v-if="anyLangAvailable(doc)"
                  :key="doc.id"
                  :name="doc.name"
                  :languages="doc.langs"
                  :doc="doc"
  ...
  beforeRouteLeave(to, from, next) {
    if (this.submitting) {
      const answer = window.confirm(
        this.$t(
          "Blockchain progress alert warning",
  ...
  methods: {
      ...
      try {
        ...
        if (response2.data.etherPK) {
          ...
          this.me =
            ...
            this.app.wallet.getCurrentAddress();

          if (
            this.me === "" ||
            this.me === null ||
            this.accountEthAddress === "" ||
            this.accountEthAddress === null
          ) {
          ...
          this.isConnectedAccount =
            this.me.toLowerCase() === this.accountEthAddress.toLowerCase();
        }
      } catch (e) {
        console.log(e);
      }
    ...
    xesTransfer(cb) {
      return new Promise((resolve, reject) => {
        this.wallet
          .approveXES(this.wallet.serviceConfig.PROXEUS_FS_ADDRESS, 1)
          .then((result) => {
            resolve(result);
          })
          .catch((e) => {
    ...
    initialScreen() {
      this.loading = true;
      axios.get("/api/document/" + this.id).then(
        (response) => {
          this.loading = false;
          ...
          try {
            ...
            if (
              ...
              this.$notify({
                ...
                text:
                  error.response.data +
                  ". Please try again or if the error persists contact the platform operator.",
                type: "error",
              });
            }
            if (error.response && error.response.status === 404) {
              ...
              if (!printedError) {
                this.$notify({
                  ...
                  title: this.$t("Error"),
                  text: this.$t(
                    "An unexpected error occurred. Please try again or if the error persists contact the platform operator."
                  ),
                  type: "error",
                });
              }
    ...
    previous() {
      this.loading = true;
      axios.get("/api/document/" + this.id + "/prev").then(
        (response) => {
          this.loading = false;
          this.handleDocumentResponse(response);
        },
      ...
      axios
        ...
        .then(
          ...
          (err) => {
            ...
            err.response &&
              err.response.data &&
              $(".form-compiled > form").showFieldErrors(err.response.data);

            this.$nextTick(() => {
              document.querySelector(".error") &&
                this.$scrollTo(".error", 500, {
                  container: "#docForm",
                  offset: -150,
                });
            });
      ...
      if (!this.status.data) {
        if (this.status.hasNext === false) {
          ...
          this.$notify({
            ...
            title: this.$t("Error"),
            text: this.$t(
              "Could not complete. Please try again or if the error persists contact the platform operator."
            ),
            type: "error",
          });
        }
    ...
    submitDoc() {
      ...
      this.$children.forEach((pdfPreviewComp) => {
        if (pdfPreviewComp.templateLanguage) {
          templateLanguageConfig[pdfPreviewComp.doc.id] =
            pdfPreviewComp.templateLanguage;
        }
      });
      axios
        .post(
          "/api/document/" + this.id + "/next?final",
    ...
    async confirmDoc(hash) {
      return new Promise((resolve, reject) => {
        ...
        this.nonce =
          this.wallet.proxeusFS.web3.eth.getTransactionCount(account);
        this.nonce++;
        this.wallet.proxeusFS
          .createFileUndefinedSigners({
            from: account,
            hash,
            data:
              this.data === ""
                ? "0x00"
                : this.wallet.proxeusFS.web3.utils.fromAscii(this.data),
            mandatorySigners: 0,
            expiry: 0,
            providers: [],
            nonce: this.nonce,
          ...
          .catch((error) => {
            ...
            this.$notify({
              ...
              title: this.$t("Error"),
              text: this.$t(
                "Could not register document. Please try again or if the error persists contact the platform operator."
              ),
              type: "error",
            });
            this.submitting = false;
            reject(
              new Error(
                this.$t(
                  "Could not create transaction. Please try again or if the error persists contact the platform operator."
                )
              )
            );
          });
    ...
    async confirm() {
      ...
      if (this.blockchainNet !== clientProvidedNet) {
        this.$notify({
          ...
          title: "Error",
          text:
            "Could not register document. Please switch to " +
            this.blockchainNet +
            " Network in MetaMask and try again.",
          type: "error",
          duration: 5000,
        });
        return;
      ...
      try {
        ...
        this.$notify({
          ...
          title: this.$t("Error"),
          text: this.$t(
            "Please make sure you are logged in to Metamask and refresh."
          ),
          type: "error",
        });
      }
    ...
    getFormData() {
      ...
      return $(".form-compiled > form").serializeFormToObject(
        excludeFileFormFields
      );
    },
    compile(form, cb) {
      if (this.components) {
        ...
        axios.get("/api/form/component?l=1000").then(
          (res) => {
