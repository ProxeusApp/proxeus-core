...
export default {
  ...
  methods: {
    ...
    amIWriteGrantedFor(item) {
      ...
      try {
        ...
        if (this.me.role !== 0) {
          if (
            item.group <= this.me.role &&
            item.groupAndOthers.rights[0] === 2
          ) {
    ...
    initUserHasSession(redirectTo) {
      ...
      const hasRequestedRedirect = new URLSearchParams(
    ...
    exportData(params, cb, url, name) {
      ...
      axios(
        ...
        (err) => {
          ...
          this.unblockUI();
          let text = this.$t(
            "There was an error exporting the data. Please try again or if the error persists contact the platform operator."
          );
          try {
    ...
    importData(file, skipExisting, cb) {
      ...
      axios
        .post("/api/import?skipExisting=" + skipExisting, file, {
          headers: {
        ...
        .then(
          ...
          (err) => {
            this.unblockUI();
            let text = this.$t(
              "There was an error while importing the data. Please try again or if the error persists contact the platform operator."
            );
            try {
              text = err.response.data;
            } catch (e) {}
    ...
    loadMeta(clb) {
      axios.get("/api/i18n/meta").then(
        (response) => {
          this.meta = response.data;
          if (this.meta && this.meta.langFallback) {
            this.$i18n.fallback(this.meta.langFallback);
            this.$i18n.add(
              this.meta.langFallback,
              this.meta.fallbackTranslations
            );
          }
          if (clb) {
            try {
    ...
    async setConfig(d) {
      ...
      if (this.blockchainNet && this.blockchainProxeusFSAddress) {
        this.wallet = new WalletInterface(
          this.blockchainNet,
          this.blockchainProxeusFSAddress
        );
  ...
  created() {
    ...
    // detect Ethereum plugin
    if (!window.ethereum) {
      return console.warn("No Ethereum plugin detected");
    }
