...
func Map(v interface{}, strRules string) error {
	if v != nil {
		val := _indirect(r.ValueOf(v))
		strType := val.Type()
		if strType.Kind() != r.Map {
			return fmt.Errorf("not a map! type:%v val:%v", strType, val.Interface())
		}
		...
		for _, mk := range val.MapKeys() {
			...
			if k == r.Struct || k == r.Ptr {
				err := me.validate(n, fmt.Sprintf("%v.", mk))
				if err != nil {
					return err
				}
			} else {
				if children, ok := rules[children].(Rules); ok {
					errs := Field(n.Interface(), children)
					if len(errs) > 0 {
						for _, er := range errs {
							er.I = mk
						}
						me.ErrorMap[fmt.Sprintf("%v", mk)] = errs
					}
				}
			}
		}
...
func Slice(v interface{}, strRules string) error {
	if v != nil {
		val := _indirect(r.ValueOf(v))
		strType := val.Type()
		if strType.Kind() != r.Slice && strType.Kind() != r.Array {
			return fmt.Errorf("not a slice! type:%v val:%v", strType, val.Interface())
		}
		...
		for i := 0; i < val.Len(); i++ {
			...
			k := n.Kind()
			if k == r.Struct || k == r.Ptr {
				err := me.validate(n, fmt.Sprintf("%v.", i))
				if err != nil {
					return err
				}
			} else {
				if children, ok := rules[children].(Rules); ok {
					...
					if len(errs) > 0 {
						...
						me.ErrorMap[fmt.Sprintf("%v", i)] = errs
					}
				}
			}
		}
